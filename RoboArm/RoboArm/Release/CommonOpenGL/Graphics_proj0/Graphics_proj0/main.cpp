/*	
	...
	The following code is from the OPENGL Programming Guide 7th edition (RED BOOK).
	This is an example from appendix A.
...
init( void )
{
	...
	points[i++] = vec2(x+w,y);
	//y = y - w;
	points[i++] = vec2(x+w,y-w);
	w = w + 0.05;
	points[i++] = vec2(x-w,y-w+.05);
